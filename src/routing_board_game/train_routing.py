...
def train_routing_agent(
    ...
    def make_env():
        return RoutingBoardGameEnv(
            num_ai_pieces=num_ai_pieces, max_steps=max_steps, reward_shaping=True
        )
    ...
    demo_env = RoutingBoardGameEnv(
        num_ai_pieces=num_ai_pieces, max_steps=max_steps, render_mode="human"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_ai_pieces", type=int, default=8, help="Number of AI pieces (default: 8)"
    )
    parser.add_argument(
        "--max_steps",
        type=int,
